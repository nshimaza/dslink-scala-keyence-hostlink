...
class KeyenceHostLinkDSLinkHandler() extends DSLinkHandler {
  ...
  override def onResponderInitialized(link: DSLink): Unit = {
        ...
        addPlcParams match {
          case Right((name, ipAddr, port, devName, devAddr, count)) =>
            if (superRoot.getChild(name, true) != null) {
              ...
              addStartPollingAction(plcNode)
              addStopPollingAction(plcNode)
              addRemoveAction(plcNode)
    ...
    def addStartPollingAction(plcNode: Node) = {
      plcNode.createChild("Start Polling", true)
        .setAction(new Action(Permission.CONFIG, (event: ActionResult) => {
          val name = plcNode.getName
          val startPollParams = for {
            interval <- (runningPlcMap.get(name) match {
              case None => Right(())
          ...
          startPollParams match {
            case Right(interval) =>
              runningPlcMap = runningPlcMap + (name -> createPollingStream(plcNode, interval))
    ...
    def addStopPollingAction(plcNode: Node) = {
      plcNode.createChild("Stop Polling", true)
        .setAction(new Action(Permission.CONFIG, (event: ActionResult) => {
          val name = plcNode.getName
          runningPlcMap.get(name) match {
    ...
    def addRemoveAction(plcNode: Node) = {
    ...
    def createPollingStream(plcNode: Node, interval: Int): (UniqueKillSwitch, Future[Done]) = {
      val ipAddr = plcNode.getAttribute("ipAddr").getString
      val port = plcNode.getAttribute("port").getNumber.intValue
      val devName = plcNode.getAttribute("devName").getString
      val devAddr = plcNode.getAttribute("devAddr").getNumber.intValue
      val count = plcNode.getAttribute("count").getNumber.intValue
      ...
      RestartSource.withBackoff(1.seconds, 60.seconds, 0.1) { () => pollingStreamSource }
    ...
    superRoot.getChildren.values.asScala.toSeq
      .filter(_.getName != "addPLC").filter(_.getName != "sys")
      .map { plcNode =>
        println(plcNode.getName)
        addStartPollingAction(plcNode)
        addStopPollingAction(plcNode)
        addRemoveAction(plcNode)
        plcNode
      }.filter(_.getChild("isRunning", true).getValue.getBool)
      .foreach { plcNode =>
        val interval = plcNode.getChild("pollingInterval", true).getValue.getNumber.intValue
        runningPlcMap = runningPlcMap + (plcNode.getName -> createPollingStream(plcNode, interval))
      }
  }
